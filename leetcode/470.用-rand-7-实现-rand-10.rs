 /*
 * @lc app=leetcode.cn id=470 lang=rust
 *
...
impl Solution {
    pub fn rand10() -> i32 {
        loop {
            //[1..7]生成[1..10],([1..7]-1)*7 + [1..7]可得到[1..49],
            //这里舍弃[41..49],保留[1..40]
            let mut num = (rand7() -1) * 7 + rand7();
            if num <= 40 {
                return 1 + num%10;
            }
            //再利用[41..49]生成[1..10],([41..49]-40-1)*7+[1..7]可得到[1..63],
            //这里舍弃[61..63],保留[1..60]
            num = (num - 40 - 1) * 7 + rand7();
            if num <= 60 {
                return 1 + num%10;
            }
            //再利用[61..63]生成[1..10],([61..63]-60-1)*7+[1..7]可得到[1..21],
            //这里舍弃[1],保留[1..20]
